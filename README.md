...
- **LiveKit RTC + Fish Audio** — 实时语音通话（简单语音循环 + Fish Audio ASR/TTS + 后端 Agent）
- 后续接入 Unity / 其他前端形态
...
- **实时语音对话** — 基于 LiveKit RTC (WebRTC) 的简单语音循环
- **静音检测** — RMS 阈值检测，简单可靠
- **STT 语音识别** — Fish Audio ASR，支持中英文
- **TTS 语音合成** — Fish Audio TTS，支持声音克隆与自定义音色
- **后端集成** — 复用完整 Agent 能力（PromptEngine、Persona、Memory、Knowledge、Tools）
...
| 语音通话 | LiveKit RTC / Fish Audio ASR / Fish Audio TTS / 简单语音循环 |
| 部署 | Docker Compose |
...
│       ├── agent.py              # 简单语音循环（监听→ASR→后端→TTS→播放）
│       ├── fish_audio.py         # Fish Audio ASR/TTS 异步封装
│       ├── test_voice.py         # 测试脚本
│       └── pyproject.toml        # 依赖声明（极简：livekit + httpx + numpy）
├── packages/
│   ├── common/                   # 公共工具包（配置、日志）
│   └── schemas/                  # 事件协议、DTO
├── .makima/                      # 配置文件目录
...
```bash
livekit>=0.17.0                  # LiveKit RTC SDK（WebRTC 音频传输）
httpx>=0.27.0                    # Fish Audio API 调用
numpy>=1.24.0                    # 音频处理
...
# MAKIMA_FISH_AUDIO_BASE_URL=https://api.fish.audio
...
- `services/voice-runtime/` 是语音通话服务，基于 LiveKit RTC + Fish Audio（简单语音循环）
...
A: 修改根目录 `.env` 中的 `MAKIMA_LLM_MODEL` 和 `MAKIMA_LLM_API_BASE`。支持任何 OpenAI 兼容的 API（DeepSeek、OpenAI、本地 Ollama 等）。
...
A: 在 CLI 中使用 `/fishvoices` 查看可用声音，`/fishvoice <voice_id>` 切换。或直接修改 `.env` 中的 `MAKIMA_FISH_AUDIO_REFERENCE_ID`。
...
A: 系统会自动保存对话记忆。确保根目录 `.env` 中启用了记忆功能（`MAKIMA_MEMORY_ENABLED=true`）。
