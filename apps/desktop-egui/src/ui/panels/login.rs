    ...
    ui.vertical_centered(|ui| {
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ...
                ui.add(egui::TextEdit::singleline(&mut login_state.server_url).hint_text("http://localhost:8000").desired_width(f32::INFINITY));
                ui.add_space(8.0);

                ui.colored_label(colors::TEXT_SECONDARY, "Username");
                ui.add(egui::TextEdit::singleline(&mut login_state.username).hint_text("admin").desired_width(f32::INFINITY));
                ui.add_space(8.0);

                ui.colored_label(colors::TEXT_SECONDARY, "Password");
                ui.add(egui::TextEdit::singleline(&mut login_state.password).password(true).hint_text("••••••••").desired_width(f32::INFINITY));
                ui.add_space(16.0);
                ...
                let can_login = !login_state.username.is_empty() && !login_state.password.is_empty() && !login_state.server_url.is_empty() && !login_state.loading;
                let btn_text = if login_state.loading { "Connecting..." } else { "Connect" };

                if ui.add_enabled(can_login, egui::Button::new(btn_text)).clicked() {
    ...
    egui::Frame::none().fill(colors::GRAPHITE_ELEVATED).inner_margin(egui::Margin::symmetric(12.0, 4.0))
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                if let Some(task) = &state.task.active_task { ui.colored_label(colors::SUCCESS, format!("Task: {}", task.status.label())); }

                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if let Some(msg) = &state.status_message { ui.colored_label(colors::TEXT_MUTED, msg); }
                    let tc = state.total_estimated_cost();
                    if tc > 0.0 { ui.colored_label(colors::TEXT_MUTED, format!("Total: ${:.5}", tc)); }
                    if state.settings.voice_config.tts_provider != "none" { ui.colored_label(colors::TEXT_MUTED, "🎤"); }
                });
            });
        });
}