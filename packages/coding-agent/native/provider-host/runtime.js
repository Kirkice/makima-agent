...
import { ProviderHost } from "./index.js";
import { runProviderHostStdio } from "./stdio.js";
/**
 ...
 */
export function startProviderHost() {
    ...
    function close() {
        closing ??= server.close().catch((error) => {
            process.stderr.write(`Provider Host shutdown failed: ${error instanceof Error ? error.message : String(error)}\n`);
            process.exitCode = 1;
        });
        return closing;
    }
